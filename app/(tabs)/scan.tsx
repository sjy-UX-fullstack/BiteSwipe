...
function scoreRecipe(recipeIngredients: string[], selected: string[]): number {
  const normalised = selected.map(s => s.toLowerCase());
  const matches = recipeIngredients.filter(ri =>
    normalised.some(sel => ri.toLowerCase().includes(sel) || sel.includes(ri.toLowerCase().split(' ')[0]))
  );
...
export default function ScanScreen() {
    ...
    try {
      ...
      if (!result.canceled && result.assets[0].base64) {
        ...
        try {
          ...
          alert('Failed to analyse image. Check your internet connection.');
          setScanned(false);
        } finally {
          setIsScanning(false);
        }
  ...
  const handleFindRecipes = () => {
    ...
    // Score and sort all mock recipes by ingredient match
    const scored = MOCK_RECIPES.map(r => ({
      ...r,
      matchPercentage: scoreRecipe(r.ingredients, selected),
    }))
      .filter(r => r.matchPercentage > 0)
      .sort((a, b) => (b.matchPercentage ?? 0) - (a.matchPercentage ?? 0));

    // Pass matched recipe IDs to the see-all screen
    const ids = scored.map(r => r.id).join(',');
    router.push({ pathname: '/see-all', params: { ids, title: 'Fridge Matches' } });
  };

  // Best matches shown inline (top 3 by score)
  const matchedRecipes = MOCK_RECIPES.map(r => ({
    ...r,
    matchPercentage: selected.length > 0 ? scoreRecipe(r.ingredients, selected) : (r.matchPercentage ?? 0),
  }))
    .sort((a, b) => (b.matchPercentage ?? 0) - (a.matchPercentage ?? 0))
    .slice(0, 3);
          ...
          {!scanned ? (
            <View style={s.cameraBox}>
              <LinearGradient colors={['rgba(255,107,53,0.08)', 'rgba(255,45,135,0.08)']} style={s.cameraInner}>
                ...
                <Text style={s.camHint}>Choose a photo from your gallery</Text>
              </LinearGradient>
            ...
            <View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 100 }}>
              ...
              <Text style={{ color: BrandColors.textSecondary, marginTop: 16, ...Typography.body }}>
            ...
            <>
              <GlassCard style={s.resultHeader} intensity="heavy">
                <View style={s.resultRow}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={s.resultSub}>Tap to deselect ingredients you don't have</Text>
                  </View>
                </View>
              </GlassCard>
                    ...
                    <TouchableOpacity key={item.name} onPress={() => toggleIngredient(item.name)} activeOpacity={0.7} style={s.ingCardWrapper}>
                      {isSelected ? (
                        ...
                        <GlassCard style={s.ingCardGlass}>
                          <Feather name="x" size={24} color={BrandColors.textTertiary} />
              ...
              {selected.length > 0 && (
                <TouchableOpacity activeOpacity={0.85} style={{ marginTop: Spacing.lg }} onPress={handleFindRecipes}>
                  <LinearGradient colors={Gradients.primary as [string, string]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={[s.scanBtn, Shadows.glow]}>
